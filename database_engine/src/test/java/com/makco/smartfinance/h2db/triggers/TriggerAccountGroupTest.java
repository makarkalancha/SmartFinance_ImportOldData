...
public class TriggerAccountGroupTest {
    private static final Logger LOG = LogManager.getLogger(TriggerAccountGroupTest.class);
    ...
    public static void setUpClass() throws Exception {
        String mess1 = "TriggerAccountGroupTest: Test->BeforeClass";
...
//        H2DbUtils.setSchema(dbConnectionResource.getConnection(), "TEST");
    ...
    public static void tearDownClass() throws Exception {
        ...
        String mess1 = "TriggerAccountGroupTest: Test->AfterClass";
        System.out.println(mess1);
        LOG.debug(mess1);
    }
    ...
    public void setUp() throws Exception {
        String mess1 = "TriggerAccountGroupTest: Test->Before";
        System.out.println(mess1);
        LOG.debug(mess1);
    }
    ...
    public void tearDown() throws Exception {
        String mess1 = "TriggerAccountGroupTest: Test->After";
        System.out.println(mess1);
        LOG.debug(mess1);
    }
    ...
    @Test
    public void testAccountGroup_11_insert() throws Exception {
        LOG.debug("testAccountGroup_11_insert");
        String queryDates = "SELECT " + Table.ACCOUNT_GROUP.ID + " FROM " + Table.Names.ACCOUNT_GROUP +
                " WHERE " + Table.ACCOUNT_GROUP.ID + " = ?" +
                " AND " + Table.ACCOUNT_GROUP.T_CREATEDON + " IS NOT NULL" +
                " AND " + Table.ACCOUNT_GROUP.T_UPDATEDON + " IS NOT NULL" +
                " AND " + Table.ACCOUNT_GROUP.T_CREATEDON + " = " + Table.ACCOUNT_GROUP.T_UPDATEDON;
    ...
    @Test
    public void testAccountGroup_12_insertDuplicate_Name() throws Exception {
        LOG.debug("testAccountGroup_12_insertDuplicate_Name");
        String queryDates = "SELECT " + Table.ACCOUNT_GROUP.ID + " FROM " + Table.Names.ACCOUNT_GROUP +
                " WHERE " + Table.ACCOUNT_GROUP.ID + " = ?" +
                " AND " + Table.ACCOUNT_GROUP.T_CREATEDON + " IS NOT NULL" +
                " AND " + Table.ACCOUNT_GROUP.T_UPDATEDON + " IS NOT NULL" +
    ...
    @Test
    public void testAccountGroup_13_insertDuplicate_Type() throws Exception {
        LOG.debug("testAccountGroup_13_insertDuplicate_Type");
        String queryDates = "SELECT " + Table.ACCOUNT_GROUP.ID + " FROM " + Table.Names.ACCOUNT_GROUP +
                " WHERE " + Table.ACCOUNT_GROUP.ID + " = ?" +
                " AND " + Table.ACCOUNT_GROUP.T_CREATEDON + " IS NOT NULL" +
                " AND " + Table.ACCOUNT_GROUP.T_UPDATEDON + " IS NOT NULL" +
    ...
    public void testAccountGroup_14_insertDuplicate_NameAndType() throws Exception {
        LOG.debug("testAccountGroup_14_insertDuplicate_NameAndType");
        //Unique index or primary key violation: "IDX_UNQ_CTGRGRP_TPNM ON TEST.ACCOUNT_GROUP(TYPE, NAME) VALUES ('D', 'income', 7)"
        testAccountGroup_11_insert();
    }

    public void update(Long id, String type, String name) throws Exception {
        LOG.debug("update");
        String queryUpdate = "UPDATE " + Table.Names.ACCOUNT_GROUP + " SET " +
                Table.ACCOUNT_GROUP.TYPE + " = ?, " +
                Table.ACCOUNT_GROUP.NAME + " = ? " +
                " WHERE " + Table.ACCOUNT_GROUP.ID + " = ?";
    ...
    @Test
    public void testAccountGroup_21_update() throws Exception {
        LOG.debug("testAccountGroup_21_update");
        String querySelect = "SELECT MAX(" + Table.ACCOUNT_GROUP.ID + ") FROM " + Table.Names.ACCOUNT_GROUP;
        String queryDates = "SELECT " + Table.ACCOUNT_GROUP.ID + " FROM " + Table.Names.ACCOUNT_GROUP +
                " WHERE " + Table.ACCOUNT_GROUP.ID + " = ?" +
                " AND " + Table.ACCOUNT_GROUP.T_CREATEDON + " IS NOT NULL" +
                " AND " + Table.ACCOUNT_GROUP.T_UPDATEDON + " IS NOT NULL" +
    ...
    @Test
    public void testAccountGroup_22_update_Name() throws Exception {
        LOG.debug("testAccountGroup_22_update_Name");
        String querySelect = "SELECT MAX(" + Table.ACCOUNT_GROUP.ID + ") FROM " + Table.Names.ACCOUNT_GROUP;
        String queryDates = "SELECT " + Table.ACCOUNT_GROUP.ID + " FROM " + Table.Names.ACCOUNT_GROUP +
                " WHERE " + Table.ACCOUNT_GROUP.ID + " = ?" +
                " AND " + Table.ACCOUNT_GROUP.T_CREATEDON + " IS NOT NULL" +
    ...
    @Test
    public void testAccountGroup_23_update_Type() throws Exception {
        LOG.debug("testAccountGroup_23_update_Type");
        String querySelect = "SELECT MAX(" + Table.ACCOUNT_GROUP.ID + ") FROM " + Table.Names.ACCOUNT_GROUP;
        String queryDates = "SELECT " + Table.ACCOUNT_GROUP.ID + " FROM " + Table.Names.ACCOUNT_GROUP +
                " WHERE " + Table.ACCOUNT_GROUP.ID + " = ?" +
                " AND " + Table.ACCOUNT_GROUP.T_CREATEDON + " IS NOT NULL" +
    ...
    //Unique index or primary key violation: "IDX_UNQ_CTGRGRP_TPNM ON TEST.ACCOUNT_GROUP(TYPE, NAME) VALUES ('C', 'house', 9)"
    @Test(expected=JdbcSQLException.class)
    public void testAccountGroup_24_updateDuplicate_NameAndType() throws Exception {
        LOG.debug("testAccountGroup_24_updateDuplicate_NameAndType");
        ResultSet rs = null;
        try {
            long idJustInserted = insert("C", "transport","bus, taxi");
            //"C","house" duplicate update, 24th update in method  testAccountGroup_3_update
    ...
    @Test
    public void testAccountGroup_35_delete() throws Exception {
        LOG.debug("testAccountGroup_35_delete");
        String querySelect = "SELECT MIN(" + Table.ACCOUNT_GROUP.ID + ") FROM " + Table.Names.ACCOUNT_GROUP;
        String queryDelete = "DELETE FROM " + Table.Names.ACCOUNT_GROUP + " WHERE " +
                Table.ACCOUNT_GROUP.ID + " = ?";
        String querySelectDeletedRow = "SELECT JSON_ROW FROM _DELETED_ROWS WHERE ID = (SELECT MAX(ID) FROM _DELETED_ROWS WHERE SCHEMA_NAME = 'TEST' AND TABLE_NAME = '" + Table.Names.ACCOUNT_GROUP + "')";
        ...
        try (
            ...
            long idFromDeletedRows = rowJsonObject.get(Table.ACCOUNT_GROUP.ID.toString()).getAsLong();
    ...
    private JsonObject createJsonObject() throws Exception {
        ...
        Object[] row = new Object[Table.ACCOUNT_GROUP.values().length];
        row[0] = 1L;
        row[1] = "C";
        row[2] = "health";
        row[3] = null;
        ...
        rowJson.addProperty(Table.ACCOUNT_GROUP.ID.toString(), (Long) row[0]);
        rowJson.addProperty(Table.ACCOUNT_GROUP.TYPE.toString(), (String) row[1]);
        rowJson.addProperty(Table.ACCOUNT_GROUP.NAME.toString(), (String) row[2]);
        rowJson.addProperty(Table.ACCOUNT_GROUP.DESCRIPTION.toString(), (String) row[3]);
        rowJson.addProperty(Table.ACCOUNT_GROUP.T_CREATEDON.toString(), sdfJson.format((Date) row[4]));
        rowJson.addProperty(Table.ACCOUNT_GROUP.T_UPDATEDON.toString(), sdfJson.format((Date) row[5]));

        JsonObject tableJson = new JsonObject();
        tableJson.addProperty(Table.Elements.tableName.toString(), schemaName + "." + Table.Names.ACCOUNT_GROUP);
    ...
    public void testDeleteToJsonObject() throws Exception{
        ...
        String expectedJsonString = "{\"tableName\":\"TEST.ACCOUNT_GROUP\",\"row\":{\"ID\":1,\"TYPE\":\"C\",\"NAME\":\"health\",\"DESCRIPTION\":null,\"T_CREATEDON\":\"2001-02-03 14:05:06\",\"T_UPDATEDON\":\"2006-05-04 03:02:01\"}}";
    ...
    public void testDeleteReadJsonObject() throws Exception{
        ...
        assertEquals("TEST.ACCOUNT_GROUP", schemaTableName);

        JsonObject rowJsonObject = jsonObject.get(Table.Elements.row.toString()).getAsJsonObject();

        LOG.debug("testDeleteReadJsonObject.rowJsonObject:" + rowJsonObject.toString());

        long id = rowJsonObject.get(Table.ACCOUNT_GROUP.ID.toString()).getAsLong();
        assertEquals(1L, id);
        String name = rowJsonObject.get(Table.ACCOUNT_GROUP.NAME.toString()).getAsString();
        assertEquals("health", name);
        JsonElement jsonElement = rowJsonObject.get(Table.ACCOUNT_GROUP.DESCRIPTION.toString());
        String description = JsonUtils.getNullableFromJsonElementAsString(jsonElement);
        assertEquals(null, description);
        Date createdOn = sdfJson.parse(rowJsonObject.get(Table.ACCOUNT_GROUP.T_CREATEDON.toString()).getAsString());
        assertEquals(sdfJson.parse("2001-02-03 14:05:06"), createdOn);
        Date updatedOn = sdfJson.parse(rowJsonObject.get(Table.ACCOUNT_GROUP.T_UPDATEDON.toString()).getAsString());
