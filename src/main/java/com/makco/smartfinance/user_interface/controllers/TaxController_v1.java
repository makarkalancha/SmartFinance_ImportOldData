                    ...
                    return new Task<EnumSet<ErrorEnum>>() {
                        ...
                        protected EnumSet<ErrorEnum> call() throws Exception {
                            return taxModel.savePendingTax(
                                    ...
                                    formulaLbl.getText(), denormformLbl.getText(),
...
//            taxRateCol.setCellValueFactory(new PropertyValueFactory<Tax, BigDecimal>("rate"));
    ...
    public void saveForm() {
        try {
            careTaker.saveState(new TaxFormState(nameTF.getText(), descTA.getText(), rateTF.getText(),
                    formulaLbl.getText(), denormformLbl.getText(), startDP.getValue(), endDP.getValue()));
        } catch (Exception e) {
            DialogMessages.showExceptionAlert(e);
        }
    }
    ...
    public boolean askPermissionToClose() {
        ...
        try{
            ...
            if(!StringUtils.isBlank(formulaLbl.getText())) {
                contentText.append("Formula (");
                contentText.append(formulaLbl.getText());
                contentText.append(") is not saved. ");
            }

            if(!StringUtils.isBlank(denormformLbl.getText())) {
                contentText.append("Denormalized Formula (");
                contentText.append(denormformLbl.getText());
                contentText.append(") is not saved. ");
            }
